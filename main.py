...
from llm_utils import batch_translate_to_chinese, summarize_topic, extract_key_concepts
...
# Comment column removed; Abstract_CN will be added dynamically by the LLM step
...
try:
    ...
    # ── Step 4: Extract global key concepts ───────────────────────────────────
    print("[main] Extracting key concepts …")
    key_concepts_md = extract_key_concepts(all_papers)

    # ── Step 5: Compose email body (overview + concepts only) ─────────────────
    email_body = f"# Daily arXiv Papers – {get_daily_date()}\n\n"
    for keyword in keywords:
        email_body += f"## {keyword}\n\n{topic_summaries[keyword]}\n\n"
    email_body += f"---\n\n## 🔑 Key Concepts & Keywords to Learn\n\n{key_concepts_md}\n"

    # ── Step 6: Write README.md and ISSUE_TEMPLATE.md ─────────────────────────
    with open("README.md", "w", encoding="utf-8") as f_rm, \
        ...
        f_rm.write(concepts_section)
        f_is.write(concepts_section)

        # ══════════════════════════════════════════════════════════════════════
        # PART 3 – Paper Details (title + bilingual abstract + authors + date)
        # ══════════════════════════════════════════════════════════════════════
        f_rm.write("---\n\n## 📄 Paper Details\n\n")
        f_is.write("---\n\n## 📄 Paper Details\n\n")
