...
def request_paper_with_arXiv_api(keyword: str, max_results: int, link: str = "OR") -> List[Dict]:
    assert link in ["OR", "AND"], "link should be 'OR' or 'AND'"
    quoted_keyword = f'"{keyword}"'
    
    url = f"http://export.arxiv.org/api/query?search_query=ti:{quoted_keyword}+{link}+abs:{quoted_keyword}&max_results={max_results}&sortBy=lastUpdatedDate"
